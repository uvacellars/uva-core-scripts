...
# This is a modified version of the script written by Cas Hoefman:
# https://github.com/cashoefman/How_To_SetUp_Go_On_Raspberry_Pi/blob/main/getgo.sh
#
# Changes:
#   - Updated the Go download URL's on lines 52 and 54
...
export GOURL="$(curl https://go.dev/dl/|grep armv6l|grep -v beta|head -1|awk -F\> {'print $3'}|awk -F\< {'print $1'})"
# Download the latest armv6 version
wget https://go.dev/dl/$GOURL
